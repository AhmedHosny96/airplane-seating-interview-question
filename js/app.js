...
var controller = {
  init: function () {
    ...
    document.getElementById("button").addEventListener("click", function () {
      ...
      let inputArrayRowsColumns = controller.transformInput(stringRowsColumns);
      result = [];

      render.clearFromDOM(seats); // removing old result
      if (controller.validateInput(inputArrayRowsColumns, queue) === false) {
        return false;
      }
      render.addBackgroundImg();
      ...
      render.createSeating(inputArrayRowsColumns, result);
    });
  },
  ...
  transformInput: function (string) {
    ...
    //remove first and last 2 symbol
  ...
  seatPassengers: function (res, que) {
    if (res.length < que) {
      alert(
        "Only the first " + res.length + " passengers will be able to fly."
      );
...
var render = {
  ...
  addBackgroundImg: function () {
    seats.style.background = "url('assets/front.png') no-repeat center top";
